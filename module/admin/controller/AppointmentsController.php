...
function filterAction()
{
    ...
    foreach ($appointments_data as $appointment_data)
    {
        ...
        elseif ($appointment->isTomorrow())
            ...
            $date_text=date('j', $appointment->getDate()).' '.$months[date('n', $appointment->getDate())-1];
        ...
        $time_text=(date('H', $appointment->getDate())==='11' ? 'об' : 'о').' '.date('G:i', $appointment->getDate());

        $appointment_text=sprintf($appointment->getOnline() ? "✅ Лікар зв'яжеться з Вами в Телеграмі %s %s\n\nДякуємо, що довіряєте нам! ❤️" : "✅ Лікар чекатиме Вас %s %s\n\nНаша адреса %s\n\n%sДякуємо, що довіряєте нам! ❤️", $date_text, $time_text, $config['address'], $notify_sms_text);
...
function saveAction()
{
    ...
    if (!count($errors))
    {
        try
        {
            ...
            $appointment->setOnline($_POST['online']==='1');
